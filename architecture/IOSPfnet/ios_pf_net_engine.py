...
from architecture.IOSPfnet.model_iosPFNet import ios_netG
...
class IOSPFNetEngine(TrainEngine):
    def train(self):
        ...
        point_netG = ios_netG(opt.num_scales, opt.each_scales_size, [4096], opt.crop_point_num)
        ...
        if opt.D_choose == 1:
            for epoch in range(resume_epoch, opt.epoch):
                ...
                for i, data in enumerate(dataloader, 0):
                    count+=1
                    label,real_point,input_cropped1,real_center,all_cropped_indices=self.process_data(data)
                    ############################
                    # (1) data prepare
                    ###########################
