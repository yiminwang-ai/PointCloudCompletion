...
from architecture.PFNet.model_PFNet import _netG
...
class Latentfeature(nn.Module):
    def __init__(self, point_scales_list,each_scales_size):
        super(Latentfeature, self).__init__()
        self.each_scales_size = each_scales_size
        self.Convlayers1 = nn.ModuleList(
            [Convlayer(point_scales_list[0]) for i in range(self.each_scales_size)])
        self.Convlayers2 = nn.ModuleList(
            [Convlayer(point_scales_list[1]) for i in range(self.each_scales_size)])
...
class PGLNetG(nn.Module):
    def __init__(self, each_scales_size, point_scales, crop_point_num):
        super(PGLNetG, self).__init__()
        self.crop_point_num = crop_point_num
        self.latentfeature = Latentfeature(point_scales, each_scales_size)
...
class PGLNetG2(nn.Module):
    def __init__(self, each_scales_size, point_scales, crop_point_num):
        super(PGLNetG2, self).__init__()
        self.crop_point_num = crop_point_num
        self.latentfeature = Latentfeature(point_scales, each_scales_size)
        self.fc1 = nn.Linear(1920, 1024)
        self.fc2 = nn.Linear(1024, 512)
        ...
        self.fc1_1 = nn.Linear(1024, 128 * 256)
        self.fc2_1 = nn.Linear(512, 64 * 16)  # nn.Linear(512,64*256) !
        self.fc3_1 = nn.Linear(256, 64 * 3)

        self.conv1_2 = torch.nn.Conv1d(256, 256, 1)
        self.conv1_3 = torch.nn.Conv1d(256, int((self.crop_point_num * 3) / 128), 1)
        self.conv2_1 = torch.nn.Conv1d(16, 6, 1)  # torch.nn.Conv1d(256,12,1) !
    ...
    def forward(self, x):
        ...
        pc2_feat = F.relu(self.fc2_1(x_2))
        #print('pc2',pc2_feat.shape)
        pc2_feat = pc2_feat.reshape(-1, 16,64)
        pc2_xyz = self.conv2_1(pc2_feat)  # 6x64 center2

        pc3_feat = F.relu(self.fc1_1(x_1))
        pc3_feat = pc3_feat.reshape(-1, 256, 128)
        ...
        pc2_xyz_expand = torch.unsqueeze(pc2_xyz, 2)
        #print('pc2',pc2_xyz_expand.shape)
        pc3_xyz = pc3_xyz.transpose(1, 2)
        pc3_xyz = pc3_xyz.reshape(-1, 128, int(self.crop_point_num / 128), 3)
        pc3_xyz = pc2_xyz_expand + pc3_xyz
        #print(pc3_xyz.shape)
...
if __name__ == '__main__':
    ...
    input1 = torch.randn(8, 4096, 3)  #64，2048，3
    input2 = torch.randn(8, 1024, 3)
    input3 = torch.randn(8, 256, 3)
    input_ = [input1, input2, input3]  #先不同采样得到2048，512，256个点
    #netG = _netG(3, 1, [4096,1024, 256], 1024)
    netG = PGLNetG(1, [4096,1024],  1024)
