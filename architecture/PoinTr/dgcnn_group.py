...
def gather_operation(points, idx):
    """
    Args:
        points: (B, C, N) tensor of point features (B: batch size, C: number of features, N: number of points)
        idx: (B, S) tensor of sampled indices (S: number of sampled points)

    Returns:
        gathered_points: (B, C, S) tensor of gathered points
    """
    B, C, N = points.shape
    S = idx.shape[1]  # Number of gathered points
    idx = idx.unsqueeze(1).expand(-1, C, -1)  # Expand idx to match the number of features
    gathered_points = points.gather(2, idx)  # Gather points based on indices
...
class DGCNN_Grouper(nn.Module):
    ...
    def fps_downsample(coor, x, num_group):
        ...
        print('xyz',xyz.shape)
        fps_idx = unified_engine.utils.farthest_point_sample(xyz, num_group)
        print('fpsidx',fps_idx.shape)

        combined_x = torch.cat([coor, x], dim=1)
        print('combined_x',combined_x.shape)
        new_combined_x = (
            (
                gather_operation(combined_x, fps_idx)
            )
        )
