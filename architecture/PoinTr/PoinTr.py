...
from architecture.PoinTr.dgcnn_group import gather_operation
from unified_engine import utils
from Transformer import PCTransformer


def fps(pc, num):
    fps_idx = utils.farthest_point_sample(pc, num)
...
class PoinTr(nn.Module):
    def __init__(self, num_pred=512,num_query=224,trans_dim=768,knn_layer=-1):
        super().__init__()
    ...
    def forward(self, xyz):
        ...
        B, M ,C = q.shape
        print('q',q.shape)
        print('coarse_point_cloud',coarse_point_cloud.shape)
        ...
        print('rebuild',rebuild_points.shape)

        ret = (coarse_point_cloud, rebuild_points)
        return ret
if __name__=="__main__":
    # num_query决定一切
    rand=torch.randn(16,4096,3)
    net=PoinTr(num_query=512)
    output=net(rand)
    print(output[0].shape,output[1].shape)
    #calculate_params.calculate(net,rand)
