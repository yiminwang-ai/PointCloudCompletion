...
from unified_engine import utils, shapenet_part_loader
...
def main():
    ...
    test_dset = shapenet_part_loader.PartDataset( root='./shape_data/',classification=True, class_choice=None, npoints=opt.pnum, split='test')
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                if opt.cropmethod == 'random_center':
                    ...
                    for m in range(batch_size):
                        ...
                        # 计算每个点与p_center之间的距离
                        # 使用向量化操作计算距离
                        distances = torch.sum((real_point[m, 0] - p_center) ** 2, dim=1)
                        ...
                        input_cropped1.data[m, 0, nearest_indices] = torch.zeros(opt.crop_point_num, 3,
                                                                                 dtype=input_cropped1.dtype)
                        # 存储最近的点到real_center
