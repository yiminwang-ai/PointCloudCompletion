...
import self_utils
from architecture.PFNet.imodules import *

import calculate_params

p= self_utils.Printf(False)
...
class NewLatentfeature(nn.Module):
    def __init__(self, num_scales, each_scales_size, point_scales_list):
        super(NewLatentfeature, self).__init__()
        self.num_scales = num_scales
        self.each_scales_size = each_scales_size
        self.point_scales_list = point_scales_list
        self.Convlayers1 = nn.ModuleList(
            [SimConvlayer(point_scales=self.point_scales_list[0]) for i in range(self.each_scales_size)])
        self.Convlayers2 = nn.ModuleList(
            [SimConvlayer(point_scales=self.point_scales_list[1]) for i in range(self.each_scales_size)])
        self.Convlayers3 = nn.ModuleList(
            [SimConvlayer(point_scales=self.point_scales_list[2]) for i in range(self.each_scales_size)])
        self.conv1 = torch.nn.Conv1d(3, 1, 1)
        self.bn1 = nn.BatchNorm1d(1)
    ...
    def forward(self, x):
        ...
        #        latentfeature = torch.squeeze(latentfeature,2)
        return latentfeature
class VeryLightLatentfeature(nn.Module):
    def __init__(self, num_scales, each_scales_size, point_scales_list):
        super(VeryLightLatentfeature, self).__init__()
        self.num_scales = num_scales
        self.each_scales_size = each_scales_size
        self.point_scales_list = point_scales_list
        self.Convlayers1 = nn.ModuleList(
            [VeryLightConvlayer(point_scales=self.point_scales_list[0]) for i in range(self.each_scales_size)])
        self.Convlayers2 = nn.ModuleList(
            [VeryLightConvlayer(point_scales=self.point_scales_list[1]) for i in range(self.each_scales_size)])
        self.Convlayers3 = nn.ModuleList(
            [VeryLightConvlayer(point_scales=self.point_scales_list[2]) for i in range(self.each_scales_size)])
        self.conv1 = torch.nn.Conv1d(3, 1, 1)
        self.bn1 = nn.BatchNorm1d(1)
...
class PointcloudCls(nn.Module):
    def __init__(self, num_scales, each_scales_size, point_scales_list, k=40):
        ...
        self.latentfeature = NewLatentfeature(num_scales, each_scales_size, point_scales_list)
        self.fc1 = nn.Linear(960, 1024)
...
class light_netG(nn.Module):
    def __init__(self, num_scales, each_scales_size, point_scales_list, crop_point_num):
        super(light_netG, self).__init__()
        #如下是最开始的参数量，就是架构最左侧的
        self.fc1_num=1024
        self.fc2_num=512
        self.fc3_num=256
        self.crop_point_num = crop_point_num
        self.latentfeature = VeryLightLatentfeature(num_scales, each_scales_size, point_scales_list)
        self.fc1 = nn.Linear(960, 640)
        # self.conv1d=nn.Conv1d(1920,1024,1,1)
        self.fc2 = nn.Linear(640, 512)
        self.fc3 = nn.Linear(512, 256)

        self.fc1_1 = nn.Linear(640, 128 * 3)
        self.fc2_1 = nn.Linear(512, 64 * 3)
        self.fc3_1 = nn.Linear(256, 64 * 3)
        ...
        #        self.bn5 = nn.BatchNorm1d(64*128)
        #
        self.conv1_1 = torch.nn.Conv1d(3, 6, 1)  #torch.nn.Conv1d(256,256,1) !
        self.conv1_3 = torch.nn.Conv1d(6, int((self.crop_point_num * 3) / 128), 1)
        self.conv2_1 = torch.nn.Conv1d(3, 6, 1)
    ...
    def forward(self, x):
        x = self.latentfeature(x)
        p.say("x.shape",x.shape)
        x_1 = F.relu(self.fc1(x))  #1024
        p.say("x_1shape",x_1.shape)
        x_2 = F.relu(self.fc2(x_1))  #512
        x_3 = F.relu(self.fc3(x_2))  #256
        ...
        pc2_feat = F.relu(self.fc2_1(x_2))#64*3
        pc2_feat = pc2_feat.reshape(-1, 3, 64)
        pc2_xyz = self.conv2_1(pc2_feat)  #(6,64) center2

        pc3_feat = F.relu(self.fc1_1(x_1))#(128*3)
        pc3_feat = pc3_feat.reshape(-1, 3, 128)#(3,128)
        pc3_feat = F.relu(self.conv1_1(pc3_feat))#(6,128)
        pc3_xyz = self.conv1_3(pc3_feat)  #(crop_point_num * 3) / 128,128   fine

        pc1_xyz_expand = torch.unsqueeze(pc1_xyz, 2)#(batch,64,1,3)
        pc2_xyz = pc2_xyz.transpose(1, 2)#(64,6)
        pc2_xyz = pc2_xyz.reshape(-1, 64, 2, 3)#(batch,64,2,3)
        pc2_xyz = pc1_xyz_expand + pc2_xyz #张量扩增 (64,2,3)
        pc2_xyz = pc2_xyz.reshape(-1, 128, 3) #(128,3)

        pc2_xyz_expand = torch.unsqueeze(pc2_xyz, 2)#(128,1,3)
        pc3_xyz = pc3_xyz.transpose(1, 2) #128,(crop_point_num * 3) / 128
        pc3_xyz = pc3_xyz.reshape(-1, 128, int(self.crop_point_num / 128), 3) #128,(crop_point_num) / 128,3
        pc3_xyz = pc2_xyz_expand + pc3_xyz #128,(crop_point_num) / 128,3
        pc3_xyz = pc3_xyz.reshape(-1, self.crop_point_num, 3)
...
if __name__ == '__main__':
    ...
    input1 = torch.randn(8, 2048, 3)  #64，2048，3
    input2 = torch.randn(8, 512, 3)
    input3 = torch.randn(8, 256, 3)
    input_ = [input1, input2, input3]  #先不同采样得到2048，512，256个点
    ...
    calculate_params.calculate(netG,input_)


#原始 FLOPs: 16969081856.0, GFLOPs: 16.969, Param_num: 76570660, MParam_num:76.571M, model_size_MB:292.094MB
#verylight FLOPs: 4025187840.0, GFLOPs: 4.025, Param_num: 2000222, MParam_num:2.000M, model_size_MB:7.630MB
