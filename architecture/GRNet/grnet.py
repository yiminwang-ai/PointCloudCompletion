...
from architecture.GRNet.extensions.gridding import GriddingReverse, Gridding
from architecture.GRNet.extensions.cubic_feature_sampling import CubicFeatureSampling
...
class GRNet(torch.nn.Module):
    ...
    def forward(self, data):
        partial_cloud = data
        print('partial_cloud',partial_cloud.size())
        # print(partial_cloud.size())     # torch.Size([batch_size, 2048, 3])
        pt_features_64_l = self.gridding(partial_cloud)
        print(pt_features_64_l.shape)
        pt_features_64_l=pt_features_64_l.view(-1, 1, 64, 64, 64)
        print(pt_features_64_l.shape)
