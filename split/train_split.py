...
from unified_engine import shapenet_part_loader, opt, data_processor
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, data in enumerate(dataloader, 0):
            label, real_point, input_cropped, patch, sencond_points, all_cropped_indices = data_processor.split_process_data(
                data=data, opt=get_opt(), second_nearest_points_len=256, visualize=False)
            print('indices', all_cropped_indices.shape)  # batch,pnum
            print('lables', label.shape)  # batch,4096

            # xyz 6, 3, 2048
            input = input_cropped.transpose(1, 2)
            optimizer.zero_grad()

            seg_pred, trans_feat = classifier(input)
