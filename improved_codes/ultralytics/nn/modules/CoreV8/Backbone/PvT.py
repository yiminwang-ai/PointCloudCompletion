...
class PvTBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads=2, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2).view(B, C, self.b, self.c)
...
if __name__ == '__main__':
    # q  = torch.randn(4,144,64) # B N C  风格
    q  = torch.randn(3,64,128,128) # B C H W  风格
    net = PvTBlock(dim=64)
    w = net(q) # 这里的12 * 12 可以不等于 144 是为什么？
    # print(w.shape) # torch.Size([4, 144, 64]) 
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
################# 
class CPN_PvT(nn.Module):
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)  
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c2, 1) 
        self.m = nn.Sequential(*(PvTBlock(self.c) for _ in range(n)))
    def forward(self, x):
        a, b = self.cv1(x).chunk(2, 1)
        return self.cv2(torch.cat((self.m(a), b), 1))

class C3_PvT(nn.Module):
    # C3_PvT Bottleneck with 3 convolutions
    # https://github.com/iscyy/ultralyticsPro
    def __init__(self, c1, c2, n=1, extra=2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(PvTBlock(c_) for _ in range(n)))

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))

class C2f_PvT(nn.Module):
