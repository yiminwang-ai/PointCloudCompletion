...
#            cls = np.array([cls]).astype(np.int32)
        ...
        choice = np.random.choice(len(seg), self.npoints, replace=True)#不理解为什么原来是放回模式的，
        # 其实是因为有些点数比期待的标准点数要少，通过这种方式可以避免去不够的问题

        # resample，由于点数不同，这边随机取了固定的点
...
if __name__ == '__main__':
    dset = PartDataset( root='./shape_data/',classification=True, class_choice=None, npoints=4096, split='train')
